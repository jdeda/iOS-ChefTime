...
final class IngredientTests: XCTestCase {
  ...
  func testIngredientAmountEdited() async {
    ...
    await store.send(.binding(.set(\.$ingredientAmountString, "")))
    await store.send(.binding(.set(\.$ingredientAmountString, "."))) {
      $0.ingredientAmountString = "0.0"
    }
    await store.send(.binding(.set(\.$ingredientAmountString, "foo")))
    await store.send(.binding(.set(\.$ingredientAmountString, "0.2"))) {
      $0.ingredientAmountString = "0.2"
      $0.ingredient.amount = 0.2

    }

    await store.send(.binding(.set(\.$ingredientAmountString, "0."))) {
